object TelaVenda: TTelaVenda
  Left = 0
  Top = 0
  Caption = 'Venda'
  ClientHeight = 441
  ClientWidth = 624
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  OnCreate = FormCreate
  TextHeight = 15
  object GridItens: TStringGrid
    Left = 88
    Top = 152
    Width = 320
    Height = 120
    TabOrder = 0
  end
  object ButtonAdicionarItem: TButton
    Left = 88
    Top = 112
    Width = 75
    Height = 25
    Caption = 'ButtonAdicionarItem'
    TabOrder = 1
    OnClick = ButtonAdicionarItemClick
  end
  object ButtonSalvarVenda: TButton
    Left = 368
    Top = 304
    Width = 75
    Height = 25
    Caption = 'ButtonSalvarVenda'
    TabOrder = 2
    OnClick = ButtonSalvarVendaClick
  end
  object ButtonFecharVenda: TButton
    Left = 472
    Top = 304
    Width = 75
    Height = 25
    Caption = 'ButtonFecharVenda'
    TabOrder = 3
  end
  object EditCliente: TEdit
    Left = 88
    Top = 24
    Width = 121
    Height = 23
    TabOrder = 4
    Text = 'EditCliente'
    OnKeyDown = EditClienteKeyDown
  end
  object EditProduto: TEdit
    Left = 88
    Top = 64
    Width = 121
    Height = 23
    TabOrder = 5
    Text = 'EditProduto'
    OnExit = EditProdutoExit
  end
  object EditQtdade: TEdit
    Left = 240
    Top = 64
    Width = 121
    Height = 23
    TabOrder = 6
    Text = 'EditQtdade'
  end
  object EditTotalVenda: TEdit
    Left = 88
    Top = 286
    Width = 121
    Height = 23
    TabOrder = 7
    Text = 'EditTotalVenda'
  end
  object EditPreco: TEdit
    Left = 392
    Top = 64
    Width = 121
    Height = 23
    TabOrder = 8
    Text = 'EditPreco'
  end
  object FDQuery1: TFDQuery
    Connection = FDConnection1
    SQL.Strings = (
      '')
    Left = 240
    Top = 328
  end
  object FDConnection1: TFDConnection
    Params.Strings = (
      'DriverID=FB'
      
        'Database=C:\Users\Druczkowski\Documents\Sistema Venda Eco\BD\VEN' +
        'DASECOJ.FDB'
      'Password=masterkey'
      'User_Name=sysdba'
      'Server=localhost'
      'OSAuthent=No')
    LoginPrompt = False
    Left = 328
    Top = 344
  end
end
